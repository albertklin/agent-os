 ...
 */
export function generateHookCommand(hookType: string, port: number = 3011): string {
        ...
        for (const hookType of ["PreToolUse", "PostToolUse", "Stop"] as const) {
          ...
          if (newHook) {
            ...
            const filteredHooks = existingHooks.filter(
              (h: HookDefinition) => !h.command?.includes("/api/sessions/status-update")
            );
...
export function removeAgentOsHooks(projectDir: string): boolean {
  ...
  try {
    ...
    for (const hookType of ["PreToolUse", "PostToolUse", "Stop", "Notification"]) {
